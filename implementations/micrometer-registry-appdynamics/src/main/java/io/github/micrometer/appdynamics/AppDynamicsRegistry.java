...
public class AppDynamicsRegistry extends StepMeterRegistry {
    ...
    protected void publish() {
        ...
        getMeters().forEach(meter -> meter.use(gauge -> publishObservation(meter.getId(), (long) gauge.value()),
                counter -> publishSumValue(meter.getId(), (long) counter.count()),
                timer -> publishSnapshot(meter.getId(), (MetricSnapshotProvider) timer),
                summary -> publishSnapshot(meter.getId(), (MetricSnapshotProvider) summary), this::publishLongTaskTimer,
                ...
                this::publishFunctionTimer, this::noOpPublish));
    }
